...
use aphid_code::scripting::{Action, Capabilities, PluginHost, SurfaceRender, Widget, explicit};
...
impl Fixture {
    ...
    fn host_with(&self, sink: Arc<dyn Sink>) -> common::Loaded {
...
async fn run_one_tool(host: Arc<PluginHost>, tool: &str, args: &str) -> String {
    ...
    let composition = compose(host).await;
...
fn the_plugin_loads_with_commands_tools_and_a_surface() {
    ...
    assert_eq!(host.composition.tools.len(), 4);
...
async fn adding_and_completing_a_task_renders_markdown() {
    ...
    let done = run_one_tool(Arc::clone(&host.host), "todo_done", r#"{"id":1}"#).await;
    assert!(done.contains("Marked task 1 done"), "{done}");
    assert!(done.contains("- [x] 1. write tests"), "{done}");

    let listed = run_one_tool(Arc::clone(&host.host), "todo_list", "{}").await;
...
async fn clearing_the_list_removes_pending_and_done_tasks() {
    ...
    .await;
    let _ = run_one_tool(Arc::clone(&host.host), "todo_done", r#"{"id":1}"#).await;

    let cleared = run_one_tool(Arc::clone(&host.host), "todo_clear", "{}").await;
...
fn the_surface_renders_the_list_after_it_opens() {
    ...
    assert!(matches!(
        host.render_surface(&host.surfaces()[0].spec, "todo"),
