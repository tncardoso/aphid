...
pub(crate) fn register(
    ...
    // was a top-level function. A plugin's shape now is `fn apply` → `on(…)` →
    // a closure, so every expression sits two levels deeper than the default
    // was drawn for, and an ordinary string concatenation trips it.
    engine.set_max_expr_depths(64, 64);
    engine.set_max_string_size(8 * 1024 * 1024);
    engine.set_max_array_size(100_000);
    engine.set_max_map_size(100_000);

    super::cx::register(engine);
...
fn register_storage(
    ...
    let settings = super::store::config(&caps.config_dirs, plugin);
...
fn register_http(engine: &mut Engine, caps: &Capabilities, worker: &Arc<Worker>) {
    fn respond(outcome: super::worker::Outcome) -> Map {
        let mut result = Map::new();
        result.insert("status".into(), outcome.status.into());
